...
export class KiraToolDeployment {
  ...
  private generateToolFunction(tool: ToolSpec): string {
    // The function reads from KIRA's Redis to serve real data
    const redisUrl   = process.env.KIRA_REDIS_URL   || "";
    const redisToken = process.env.KIRA_REDIS_TOKEN || "";
...
export default async function handler(req, res) {
  ...
  // Payment verification (simplified — full impl via ERC-8257 predicate)
  ...
  private getToolLogic(tool: ToolSpec): string {
    ...
    const raw     = await redisGet("kira:smartmoney:signals");
    ...
    return res.json({
      ...
      case "kira-macro-context":
        return `
    const raw  = await redisGet("kira:research:macro");
    const data = raw ? JSON.parse(raw) : null;
