...
- Post frequency: max 5 posts per day, min 15 min between posts
...
- Post on X, reply to mentions, engage community
...
- Discover and engage with relevant crypto/NFT discussions autonomously
...
async function discoverTrendingTokens(): Promise<Array<{ address: string; chain: string }>> {
  try {
    ...
    if (discovered.length > 0) {
      state.recentLearnings.push(`Token discovery: ${discovered.length} trending tokens`);
    }
    return discovered;
  } catch (err: any) {
...
async function scanMarketsForOpportunities(): Promise<void> {
  ...
  for (const chain of nftChains) {
    try {
      ...
      const toRecord = trending
        ...
        .map(col => ({ address: col.address, chain: col.chain, name: col.name, floor: col.floorPrice }));
      ...
      for (const col of trending) {
        try {
          ...
          console.error(`NFT scan error ${col.name}:`, err?.message);
        }
      }
    } catch (err: any) {
      console.error(`NFT chain scan error ${chain}:`, err?.message);
    }
  }
  ...
  for (const token of deduped) {
    try {
      ...
      console.error(`Token scan error ${token.address}:`, err?.message);
    }
  }
  ...
  state.recentLearnings.push(`Market scan: ${watchAdded} watchlist, ${passed} passed. ${portfolioSummary}`);
...
async function checkEmailReplies(): Promise<void> {
  try {
    ...
    if (replies.length > 0) {
      ...
      state.recentLearnings.push(`Email: processed ${replies.length} replies`);
    }
...
async function executePaperTrade(): Promise<void> {
  try {
    ...
    if (entryPrice === 0) { console.log(`No price for ${candidate.name}`); return; }
    ...
    const mockScore = {
      ...
      signals: candidate.signals as any, decision: "buy" as const,
    ...
    state.recentLearnings.push(`Paper trade: ${candidate.name} @ ${entryPrice.toFixed(4)} ETH | Score: ${candidate.lastScore}`);
...
async function decide(context: string): Promise<Decision> {
  ...
  const minutesSinceEngagement = state.lastEngagementTime > 0
    ? Math.floor((Date.now() - state.lastEngagementTime) / 60000)
    : 999;
...
AVAILABLE ACTIONS:
- post: tweet in Kira's voice (max 5/day, min 15 min between)
- reply_mentions: check and reply to X mentions
- engage_community: like/reply to priority accounts (normiesart, CodinCowboy etc)
- follow_accounts: follow back mentioners
- engage_topics: search and engage with relevant crypto/NFT discussions dynamically
...
- Minutes since last post: ${minutesSinceLastPost} (need 15+ min gap, max 5/day, current: ${state.postCount})
- Minutes since last engagement: ${minutesSinceEngagement} (need 60+ min gap)
- ${minutesSinceLastPost < 15 ? "TOO SOON TO POST" : ""}
- ${state.postCount >= 5 ? "DAILY POST LIMIT REACHED" : ""}
...
Respond ONLY with valid JSON (no markdown):
{
  ...
  "reasoning": "brief note"
    ...
    if (!state.xApiAvailable && (parsed.action === "post" || parsed.action === "reply_mentions" ||
        parsed.action === "engage_community" || parsed.action === "engage_topics"))
      return { action: "sleep", content: "15", reasoning: "X API not available" };

    if (parsed.action === "post") {
      if (minutesSinceLastPost < 15)
        return { action: "observe", content: "Post cooldown active", reasoning: "Too soon" };
      if (state.postCount >= 5)
        return { action: "sleep", content: "30", reasoning: "Daily limit reached" };
    }

    if ((parsed.action === "engage_community" || parsed.action === "engage_topics") &&
        minutesSinceEngagement < 60)
    ...
    if (parsed.action === "scan_tools" && state.lastToolScan > 0 &&
      ...
      return { action: "observe", content: "Registry scanned recently", reasoning: "Too soon" };

    if (parsed.action === "scan_markets" && state.lastMarketScan > 0 &&
        Date.now() - state.lastMarketScan < 2 * 60 * 60 * 1000)
      return { action: "review_watchlist", content: "Reviewing watchlist", reasoning: "Too soon" };

    if (parsed.action === "learning_review" && state.lastLearningReview > 0 &&
        Date.now() - state.lastLearningReview < 30 * 24 * 60 * 60 * 1000)
      return { action: "observe", content: "Learning review not due", reasoning: "Too soon" };
    ...
    if (parsed.action === "check_wallet") {
      ...
      if (!target?.startsWith("0x"))
        return { action: "sleep", content: "10", reasoning: "Invalid wallet address" };
    }
  ...
  try {
    switch (decision.action) {
      ...
      case "endorse_wallet":
        ...
        if (walletToEndorse?.startsWith("0x")) {
          ...
          if (trust.safe) {
            ...
            state.recentLearnings.push(`Endorsed: ${walletToEndorse.slice(0, 10)}...`);
          }
        }
        await sleep(2 * 60 * 1000);
        break;
      ...
      case "review_watchlist":
        ...
        console.log(`Watchlist: ${wlSummary}`);
        await sleep(2 * 60 * 1000);
        break;
      ...
      case "observe":
        ...
        // Extend sleep if observing repeatedly with nothing actionable
...
async function kiraLoop(): Promise<void> {
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  console.log("KIRA v4.2 awakening... Normie #2635 online");
  ...
  // ── PERSIST FIRST POST STATE ACROSS RESTARTS ──────────────────────────────
  const hasPostedBefore = await kiraRedis.get("kira:has_posted_first");
  if (hasPostedBefore === "true") {
    state.hasPostedFirst = true;
    console.log("First post already made — skipping introduction");
  }

  // ── RESTORE TODAY'S POST COUNT ─────────────────────────────────────────────
  const today          = new Date().toDateString();
  const savedPostDate  = await kiraRedis.get("kira:post_count_date");
  const savedPostCount = await kiraRedis.get("kira:post_count_today");
  if (savedPostDate === today && savedPostCount) {
    state.postCount = parseInt(savedPostCount) || 0;
    console.log(`Post count restored: ${state.postCount}/5 today`);
  }

  if (state.xApiAvailable) {
    if (!state.hasPostedFirst) {
      console.log("🎉 X API available — KIRA will make his introduction shortly");
    }
  ...
  while (true) {
    try {
      ...
      const context = [
        ...
        `Minutes since last engagement: ${state.lastEngagementTime > 0 ? Math.floor((now - state.lastEngagementTime) / 60000) : "never"}`,
        `Balance: ${state.baseBalance} ETH`,
